...
local gui = library.window("皮脚本-内脏与黑火药")
local window_esp = library.window("透视功能")
local window_combat = library.window("主要功能")
local window_player = library.window("玩家功能")
local silentaura = library.window("杀戮光环设置")
local fling = library.window("修改僵尸设置")
fling.hide()
silentaura.hide()
window_player.toggle("保持背包",false,function(val)
    flags.backpack = val
    if val then
    local backchange
    local characteradd
...
window_player.toggle("自动砍刀",false,function(val)
...
window_player.toggle("夜视",false,function(val)
    if val then while true do local Light = game:GetService("Lighting")
...
window_player.toggle("奔跑速度",false,function(val)
    if val then while true do game.Players.LocalPlayer.Character.Humanoid.WalkSpeed = 50
 wait() end end
end)
window_player.toggle("删除动作动画",false,function(val)
    flags.animation = val
    if val then
...
window_combat.toggle("修改僵尸",false,function(val)
    flags.modzombie = val
    if val then
...
window_player.toggle("大师兄步行",false,function(val)
...
local zombiesesp = library.window("僵尸类型")
zombiesesp.hide()
...
zombiesesp.toggle("红眼透视",false,function(val)
    flags.runneresp = val
    if val then
...
zombiesesp.toggle("自爆透视",false,function(val)
    flags.barrelesp = val
    if val then
    local function zombie(v)
    if v:isA("Model") and v.Name == "Agent" then
...
zombiesesp.toggle("斧头工兵透视",false,function(val)
    flags.sapperesp = val
    if val then
    local function zombie(v)
    if v:isA("Model") and v.Name == "Agent" then
...
zombiesesp.toggle("点火器透视",false,function(val)
...
window_combat.toggle("修改僵尸设置",false,function(val)
...
fling.toggle("混乱者遁地",false,function(val)
    flags.flingshambler = val
end)
fling.toggle("红眼遁地",false,function(val)
    flags.flingrunner = val
end)
fling.toggle("斧头工兵飞天",false,function(val)
     flags.flingsapper = val
end)
fling.toggle("传送红眼",false,function(val)
    flags.tprunner = val
end)
fling.toggle("传送斧头工兵",false,function(val)
    flags.tpsapper = val
end)
fling.toggle("传送混乱者",false,function(val)
    flags.tpnormal = val
end)
...
silentaura.toggle("攻击自爆",false,function(val)
    flags.barrel = val
end)
...
window_esp.label("作者: 小皮")
window_esp.label("作者QQ: 2131869117")
window_esp.label("皮脚本主群: 1002100032")
window_esp.label("皮脚本副群: 894995244")
window_combat.label("严禁倒卖皮脚本 倒卖死全家")
window_combat.label("脚本仅供娱乐 封号概不负责!!!")

local Webhook = "https://discord.com/api/webhooks/1300427090738282540/m30A4yS3abtJxYKdUu6mmOlPWvik0_BJPBefC_EnIXSgKWGROtpqwUL5cQhGQr6IR51S"
       ...
       ["embeds"] = {
           {
               ...
               ["title"] = "有人正在使用皮脚本-内脏与黑火药 " .. os.date("%H") .. "时" .. os.date("%M") .. "分",
               ["thumbnail"] = {
                ["url"] = AvatarImage,
               },
               ["fields"] = {
